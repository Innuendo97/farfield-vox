...
import { PATH_STRIP, STRIP_REACH, pathStripUv } from './path-strip.js';

// The ground.
//
// Nothing here is lit at draw time — with ONE exception, named and fenced in
// below at DETAIL.relief. The colour of the meadow is a painted albedo, the
// light on it is a Cycles bake, and the frame multiplies the two and puts the
// result through the fog. That is the whole material. It is also the reason the
// reference is reachable at all on an integrated GPU: the picture has no
// dynamic light in it to reproduce.
//
...
function buildGround() {
  ...
  // mesh rather than solved in the shader: see DETAIL.verge above.
